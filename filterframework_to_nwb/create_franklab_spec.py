...
#
# The code in this notebook will generate two YAML (.yaml) files at the locations defined in the 'namespace_path' and 'extension_path' variables below. If you wanted to, you could just write these .yaml files by hand, following conventions in the [NWB Specification Language](https://schema-language.readthedocs.io/). But this would be very cumbersome! Thankfully the PyNWB API includes a number of helper functions to allow you to create these files programmatically, which is what we will demonstrate here.
#
...
# This notebook will create two YAML (.yaml) files at the locations defined in the 'namespace_path' and 'extension_path' variables below.

# Save yamls in the current directory
name = 'franklab'
namespace_filename = name + ".namespace.yaml"
extension_filename = name + ".extensions.yaml"

# The paths below are also 'exported' in __init__.py and so are available to users as:
#   franklabnwb.fl_namespace_path
#   franklabnwb.fl_extension_path
...
def main():
    ...
    task = NWBGroupSpec(neurodata_type_def='Task',              # define the new Task "neurodata_type"
                        ...
                        attributes=[NWBAttributeSpec(name='name', doc='the name of this task', dtype='text'),
                                    NWBAttributeSpec(
                                        name='description', doc='description of this task', dtype='text'),
    ...
    namespace_builder = NWBNamespaceBuilder(name + ' extensions', name)
