...
import filterframework_to_nwb.nwb_query as query
from filterframework_to_nwb.fl_extension import (Apparatus, Edge, PointNode,
...
def plot_position_by_epochs(animal, pos_series, epoch_ivls, epochs, title):
    ...
    for epoch in epochs:
        ...
        plt.plot(epoch_pos.samples['x'], epoch_pos.samples['y'],
                 label='epoch%s' % epoch, zorder=1)
...
def overlay_apparatus_geom(fig, points, segments, polygons):
    ...
    for name, point in points.items():
        plt.scatter(point[0], point[1], marker='o', edgecolors='r',
                    s=100, facecolors='none', linewidth=2, zorder=3)
    for name, segment in segments.items():
        plt.plot([c[0] for c in segment], [c[1]
                                           for c in segment], marker='o', color='k', zorder=3)
    for name, polygon in polygons.items():
        for i in range(len(polygon) - 1):
            plt.plot([polygon[i][0], polygon[i + 1][0]],
                     [polygon[i][1], polygon[i + 1][1]], color='k', zorder=2)
        plt.plot([polygon[i + 1][0], polygon[0][0]],
                 [polygon[i + 1][1], polygon[0][1]], color='k', zorder=2)
...
def nx_to_fl_node(node_name, attrs):
    ...
    if attrs['kind'] == 'segment':
        ...
        if 'interior_coords' not in attrs:
            raise TypeError(
                "NX 'polygon' nodes must contain a 'interior_coords' field. It can be set to None.")
...
def add_fl_node_to_nx_graph(fl_node, nx_graph):
    ...
    if isinstance(fl_node, SegmentNode):
        ...
        nx_graph.add_node(fl_node.name, coords=fl_node.coords,
                          interior_coords=fl_node.interior_coords, kind='polygon')
    else:
        print(fl_node.__class__)
        raise TypeError(
            "'fl_node' must be of type SegmentNode, PointNode, or PolygonNode")
    return nx_graph
...
def plot_nx_appar_geom(nx_graph, ax=None, label_nodes=True):
    ...
    for n, attrs in list(nx_graph.nodes.data()):
        if attrs['kind'] == 'point':
            ...
            raise TypeError(
                "Nodes must have 'kind' point, segment, or polygon.")
    return ax
