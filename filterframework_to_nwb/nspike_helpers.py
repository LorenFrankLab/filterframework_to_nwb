...
def parse_franklab_linpos_data(data_dir, animal, day):
    '''
    Purpose:
        Parse the 'linpos' file from Frank Lab Filter Framework for a given animal and day.
        This contains data for linearized animal position, head direction, and speed. By Frank Lab
        conventions, this file should be named 'XYZpos04.mat' for animal 'XYZ' day 4
        (e.g. 'bonpos04.mat').
    Argument:
        data_dir: full path to the directory containing the 'pos' files
...
def get_exposure_num(epoch_metadata):
    ...
    Returns:
        ...
        Returns -1 if there is no 'exposure" field in the epoch task structure
    '''
...
def loadmat_ff(filename, varname):
    ...
    if isinstance(data, np.ndarray):
        ...
        warnings.warn(
            "Matlab variable is not an ndarray, returning as-is; type = %s" % type(elem))
        return (data)
    ...
    if cellvec_to_dict and arr.shape[0] == 1:
        ...
        for idx, elem in enumerate(arr[0, :]):
            ...
            if isinstance(elem, sio.matlab.mio5_params.mat_struct):
                ...
                warnings.warn(
                    "Array element is not a mat_struct, not an ndarray; type = %s" % type(elem))
                d[key] = elem
        return d
...
def build_day_eeg(files_by_tetrode, samprate):
    ...
    for file_info in files_by_tetrode:
        ...
        time_list.append(np.ndarray.flatten(np.asarray(eeg['starttime'] +
                                                       (np.arange(0, len(eeg['data'])).reshape(-1, 1) / samprate), np.float64)))
