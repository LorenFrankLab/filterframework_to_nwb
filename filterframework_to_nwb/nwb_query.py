...
class TimeBasedData(ABC):
    ...
    def time_query(self, query):
        raise NotImplementedError(
            "Subclasses of abstract class Data must implement 'time_query' method.")
    ...
    def valid_intervals(self, new_valid_intervals):
        if not(isinstance(new_valid_intervals, TimeIntervals)):
            raise TypeError(
                "'valid_intervals' must be of type nwb_query.TimeIntervals")
...
class PointData(TimeBasedData):
    ...
    def __init__(self, point_times, valid_intervals, marks=None):
        # valid_intervals is typechecked in TimeBasedData
        self.typecheck(point_times, marks=marks)
    ...
    def typecheck(self, point_times, marks=None):
        ...
        if not(point_times.ndim == 1):
            raise ValueError(
                "'point_times' must be a vector (1-dimensional array).")
        ...
        if marks and not(marks.shape[0] == self.point_times.shape[0]):
            raise ValueError(
                "'marks' must have same # of entries (rows) as 'point_times'.")
...
#         valid_indices = [i for (i, t) in enumerate(self.point_times) if t in query]
    ...
    def mark_with_ContinuousData(self, continuous_data, merge_valid_intervals=True, interpolation='linear'):
        ...
        if continuous_data.samples.ndim > 1 and not (interpolation == 'nearest' or interpolation == 'linear'):
            raise NotImplementedError(
                "For data > 1-D, only 'nearest' and 'linear' interpolation are currently suppported")
        ...
        # 1 row per time point, but mark data can be multi-d
        mark_shape = continuous_data.samples.shape[1:]
        ...
        if merge_valid_intervals:
            ...
            result_pp.marks = pd.DataFrame(
                data=result_marks, columns=continuous_data.samples.columns)
            return result_pp
        else:
            result_marks = []
...
class ContinuousData(TimeBasedData):
    ...
    def __find_valid_intervals(self, sample_times, gap_threshold_samps=1.5):
        ...
        import warnings
        warnings.warn(
            "Deducing valid_interval is currently untested, may be bogus.")
        # use first derivatives to estimate the stepsize
        stepsize = np.mean(np.diff(sample_times, 1))
        # use second derivative to identify gaps
        diffs = np.diff(sample_times, 2)
        # only count if the gap is big with respect to the stepsize
        epsilon = gap_threshold_samps * stepsize
        ...
        if ivl_end_indices.size == 0:  # no gaps in observation
            ...
            for i, end_idx in enumerate(ivl_end_indices):
                if i == 0:   # handle the first interval
                    bounds.append(
                        [self.sample_times[0], self.sample_times[end_idx]])
                else:
                    previous_end_idx = ivl_end_indices[i - 1]
                    new_start_idx = previous_end_idx + 1
                    bounds.append(
                        [self.sample_times[new_start_idx], self.sample_times[end_idx]])
    ...
    def time_query(self, query):
        ...
        if isinstance(query, EventData):
            ...
            result_valid_intervals = self.valid_intervals & TimeIntervals(
                query)
        else:
            raise TypeError(
                "'query' must be of type nwb_query.EventData, nwb_query.TimeIntervals, or numpy.ndarray")
        ...
        result_lower_index = np.searchsorted(
            self.sample_times, result_lower_bounds, side='left')
        result_upper_index = np.searchsorted(
            self.sample_times, result_upper_bounds, side='right')
        ...
        for idx_lower, idx_upper in zip(result_lower_index, result_upper_index):
            ...
            result_samples = pd.concat(
                [result_samples, samples_next_valid_interval], ignore_index=True)
        # concatenate all the sample times
        result_sample_times = np.concatenate(result_sample_times)
    ...
    def data_query(self, query_columns):
        ...
        if not (isinstance(query_columns, list) or isinstance(query_columns, np.ndarray)):
            raise TypeError(
                "'query_columns' must be a list or numpy array of column names")
        for col in query_columns:
            if not isinstance(col, str):
                raise TypeError(
                    "All column names in 'query_columns' must be strings.")
            if col not in self.samples.columns:
                raise ValueError(
                    "{} not in the ContinuousData samples.".format(col))
    ...
    def filter_intervals(self, func):
        ...
        if func_of_data[-1]:
            down_crossings = np.append(
                down_crossings, func_of_data.shape[0] - 1)
...
class EventData(TimeBasedData):
    ...
    def __init__(self, event_intervals, valid_intervals):
        if not isinstance(event_intervals, TimeIntervals):
            raise TypeError(
                "'event_intervals' must be a query.TimeIntervals instance")
        if not np.all([event_ivl in valid_intervals for event_ivl in event_intervals.intervals]):
            raise ValueError(
                "'event_intervals' must be fully contained in 'valid_intervals'.")
...
def plot_ContinuousData(ContinuousData, axis=None, interval_pad_factor=1.1, ivl_color='b'):
    ...
    datarange = [ContinuousData.samples.values.min(
    ),  ContinuousData.samples.values.max()]
    ivl_pad = np.diff(datarange) * (interval_pad_factor - 1) / 2
    ivl_y = [datarange[0] - ivl_pad, datarange[1] + ivl_pad]

    ivl_h = plot_TimeIntervals(
        ContinuousData.valid_intervals, ivl_y, axis=axis)
...
def plot_EventData(EventData, ypos=1, axis=None, valid_color='b', color='k'):
    ...
    ivl_h = []
    ivl_h.append(plot_TimeIntervals(EventData.valid_intervals,
                                    axis=axis, ivl_y=(ypos - 0.5, ypos + 0.5), color=valid_color))
    ivl_h.append(plot_TimeIntervals(EventData.event_intervals,
                                    axis=axis, ivl_y=(ypos - 0.25, ypos + 0.25), color=color))
